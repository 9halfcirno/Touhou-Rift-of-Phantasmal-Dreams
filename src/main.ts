...
TH.MouseInput.onWheel((wheel) => {
  if (game.scene.three.camera === game.scene.currentCamera) {
    (TH.Config as unknown as Record<string, number>).object2d_tilt += wheel.y * 0.01;
  }
});
...
async function init() {
  ...
  (enemy as { position: { set: (x: number, y: number, z: number) => void } }).position.set(4, 0, 4);
